...
def find_routes(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if len(all_ways) == 0:
                messages.error(request, 'Маршрута, удовлетворяешего условием не существует')
                return render(request, 'routes/home.html', {'form': form})
            if across_cities_form:
                ...
                if not right_ways:
                    messages.error(request, 'Маршрут, через эти города не возможет')
                    return render(request, 'routes/home.html', {'form': form})
            else:
                right_ways = all_ways
            ...
            for route in right_ways:
                ...
                if total_time <= traveling_time:
                    trains.append(tmp)
            if not trains:
                messages.error(request, 'Время в пути, больше заданого')
                return render(request, 'routes/home.html', {'form': form})
            routes = []
